...
char* Encoder::statsString(EncStats& stat, char* buffer, int iBufferSize)
{
    double fps = (double)m_param->fpsNum / m_param->fpsDenom;
    double scale = fps / 1000 / (double)stat.m_numPics;

    int len = snprintf(buffer, iBufferSize, "%6u, ", stat.m_numPics);

    len += snprintf(buffer + len, iBufferSize - len, "Avg QP:%2.2lf", stat.m_totalQp / (double)stat.m_numPics);
    len += snprintf(buffer + len, iBufferSize - len, "  kb/s: %-8.2lf", stat.m_accBits * scale);
    if (m_param->bEnablePsnr)
    {
        len += snprintf(buffer + len, iBufferSize - len, "  PSNR Mean: Y:%.3lf U:%.3lf V:%.3lf",
    ...
    {
        snprintf(buffer + len, iBufferSize - len, "  SSIM Mean: %.6lf (%.3lfdB)",
...
void Encoder::printSummary()
{
    ...
    if (m_analyzeAll.m_numPics)
    {
        ...
        if (m_param->bEnableSsim)
            p += snprintf(buffer + p, sizeof(buffer) - p, ", SSIM Mean Y: %.7f (%6.3f dB)", m_analyzeAll.m_globalSsim / m_analyzeAll.m_numPics, x265_ssim2dB(m_analyzeAll.m_globalSsim / m_analyzeAll.m_numPics));
...
void Encoder::getStreamHeaders(NALList& list, Entropy& sbacCoder, Bitstream& bs)
{
    ...
    if (m_param->bEmitInfoSEI)
    {
        ...
        if (opts)
        {
            ...
            int iBufferSize = strlen(opts) + strlen(PFX(version_str)) +
                strlen(PFX(build_info_str)) + 200;
            char *buffer = X265_MALLOC(char, iBufferSize);
            if (buffer)
            {
                snprintf(buffer, iBufferSize, "x265 (build %d) - %s:%s - H.265/HEVC codec - "
